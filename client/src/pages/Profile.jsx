...
export default function Profile() {
	...
	const handleFileUpload = file => {
		//app is taken from firebase.js
		const storage = getStorage(app)
		//date is added to fileName to prevent the error of storing two files with same name. date will create two files uniquiely with same name
		const fileName = new Date().getTime() + file.name
		const storageRef = ref(storage, fileName)
		//uploadBytesResumable is a method of firebase which allows live counting of uploading of file in the website
		const uploadTask = uploadBytesResumable(storageRef, file)

		//this is going to tell us about the precentage of uploading and any error for that state_changed is used
		uploadTask.on(
			"state_changed",
			//snapshot is a peice of info from each state change
			snapshot => {
				//this shows the upload percentage
	...
	return (
		<div className="p-3 max-w-lg mx-auto">
			...
			<form onSubmit={handleSubmit} className="flex flex-col gap-4">
				{/* here ref of input is taken and passed it to img tag for updating the profile image */}
				{/* only image files are accepted using accept attribute */}
				...
				<div className="flex flex-col gap-4">
					...
					{userListings.map(listing => (
						<div key={listing._id} className="border rounded-lg p-3 flex justify-between items-center gap-4">
							...
							{/* truncate means if the name is long like saduhfausdhgasudgfasiud then it will show saduhfa... */}
							<Link className="text-slate-700 font-semibold flex-1 hover:underline truncate" to={`/listing/${listing._id}`}>
								<p>{listing.name}</p>
							</Link>
							<div className="flex flex-col items-center">
								{/* as we need to pass id inside the function we have to convert the onClick function into callBack function */}
